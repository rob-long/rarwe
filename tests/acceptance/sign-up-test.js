...
module('Acceptance | Sign up', function(hooks) {
  ...
  test('Sign up server-side errors', async function(assert) {
    await visit('/sign-up');
    await fillIn('#email', 'taken@tcv.com');
    await fillIn('#password', 'ThemCr00ked!');
    await click('[data-test-rr=sign-up-button');
    assert.dom('[data-test-rr=email-error]').hasText('email - has already been taken', 'Email taken error is displayed');
  });
});
