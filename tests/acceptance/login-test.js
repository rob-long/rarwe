...
module('Acceptance | Login', function(hooks) {
  ...
  test('Failing login', async function(assert) {
    await visit('/login');
    ...
    assert.dom('[data-test-rr=password-error]').hasText('', 'Password error is no longer there');
  });

  test('Login general server-side errors', async function(assert) {
    await visit('/login');
    await fillIn('#email', 'error500@tcv.com');
    await fillIn('#password', 'qwertyuiop');
    await click('[data-test-rr=login-button]');

    assert.dom('[data-test-rr=error-message]').hasText('Something went wrong, sorry.', 'General error is shown');
  });
});
